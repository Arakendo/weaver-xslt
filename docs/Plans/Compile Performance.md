...
[Fallback File Size Findings](Fallback%20File%20Size.md): that page is about how
...
2. **IR compile** — parse the composed XML and lower it to `StylesheetIR`,
   ...
   ([compileStylesheet](../../src/xslt/compile/compiler.ts)).
3. **Emit module** — render the generated TypeScript. For stylesheets outside the
   ...
   the module ~34 MB ([emitStylesheetModule](../../src/xslt/codegen/emit.ts)).
...
5. **Analyze + sort diagnostics** — run analyzers over the IR and order the
   results ([analyzeStylesheet](../../src/xslt/compile/analyze.ts),
   [sortDiagnostics](../../src/diagnostics/index.ts)).
...
- **Fallback IR serialization.** The fallback path serializes the entire
  ...
  (see [Fallback File Size Findings](Fallback%20File%20Size.md)), so the object
...
- Reducing the 34 MB module size — that is owned by
  [Fallback File Size Findings](Fallback%20File%20Size.md). Size and speed
