...
export function compileApplyTemplatesInstruction(
  ...
  if (mode !== null) {
    throw helpers.createXsltStaticError(
      ...
      getAttributeValueSourceLocation(stylesheetXml, element, 'mode', helpers.stylesheetSourceName)
        ?? getNodeSourceLocation(stylesheetXml, element, helpers.stylesheetSourceName),
      {
        suggestions: [{
          kind: 'fix',
  ...
  return {
    ...
    ...(select === undefined ? {} : { select: helpers.parseXPathInContext(select, location, 'xsl:apply-templates', 'select') }),
  };
}
...
export function compileIfInstruction(
  ...
  const location = getAttributeValueSourceLocation(stylesheetXml, element, 'test', helpers.stylesheetSourceName)
    ?? getNodeSourceLocation(stylesheetXml, element, helpers.stylesheetSourceName);
...
export function compileChooseInstruction(
  ...
  for (const child of helpers.childElements(element)) {
    if (helpers.isXsltElement(child, 'when')) {
      ...
      const location = getAttributeValueSourceLocation(stylesheetXml, child, 'test', helpers.stylesheetSourceName)
        ?? getNodeSourceLocation(stylesheetXml, child, helpers.stylesheetSourceName);
      whenBranches.push({
...
export function compileForEachInstruction(
  ...
  const location = getAttributeValueSourceLocation(stylesheetXml, element, 'select', helpers.stylesheetSourceName)
    ?? getNodeSourceLocation(stylesheetXml, element, helpers.stylesheetSourceName);
