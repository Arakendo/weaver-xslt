import { FORX0002 } from '../../errors/codes.js';

import { createRegexError, type RegexSpanLike } from './regexDiagnostics.js';

export const XML_NAME_START_CHAR_CLASS =
  ':A-Z_a-z\\xC0-\\xD6\\xD8-\\xF6\\xF8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD\\u{10000}-\\u{EFFFF}';
...
const SCHEMA_PROPERTY_CLASS_ALIASES: Record<string, string> = Object.fromEntries(
  SCHEMA_PROPERTY_BLOCK_ALIAS_ENTRIES,
);

export function translateCharacterClass(
  pattern: string,
  startIndex: number,
  span: RegexSpanLike,
...
function translateCharacterClassContent(content: string, span: RegexSpanLike): string {
  ...
  if (body.length === 0) {
    throw createRegexError(
      FORX0002,
      'Empty character classes are not valid in the current regex slice.',
      span,
    );
  }
...
function splitTopLevelCharacterClassSubtraction(
  content: string,
): { base: string; subtract: string } | undefined {
  let escaped = false;
  let nestedCharacterClassDepth = 0;
...
function findMatchingCharacterClassRange(
  content: string,
  startIndex: number,
...
export function translateXmlNameEscape(escape: 'i' | 'I' | 'c' | 'C'): string {
  switch (escape) {
    case 'i':
...
export function translateXPathRegexEscape(
  escape: 'd' | 'D' | 'w' | 'W',
...
export function parsePropertyEscape(
  pattern: string,
  startIndex: number,
): PropertyEscape | undefined {
...
export function translatePropertyEscape(
  propertyEscape: PropertyEscape,
  inCharacterClass: boolean,
): string {
