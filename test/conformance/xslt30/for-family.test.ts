...
const overlay = loadXslt30Overlay(
  join(REPO_ROOT, 'corpus', 'overlays', 'xslt30', 'weaver-for-family-v1.json'),
);
const selectedCases = overlay.cases.filter((testCase) => testCase.selection === 'selected');

describe('W3C conformance — complete for-expression family', () => {
  if (!hasXslt30Catalog()) {
  ...
  it('passes selected cases and conserves three explicit engine gaps', () => {
