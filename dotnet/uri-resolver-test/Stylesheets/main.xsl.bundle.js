  ...
  "node_modules/@xmldom/xmldom/lib/sax.js"(exports) {
        ...
        try {
          ...
          if (tagStart < 0) {
            ...
            if (!source2.substring(start).match(/^\s*$/)) {
              ...
              var text = doc.createTextNode(source2.substring(start));
    ...
    function parseUtils(source2, start) {
      ...
      function substringStartsWith(text) {
        return source2.substring(index, index + text.length) === text;
      }
      function substringStartsWithCaseInsensitive(text) {
        return source2.substring(index, index + text.length).toUpperCase() === text.toUpperCase();
      }
...
function parseXml(source2, options = {}) {
  const stripped = source2.charCodeAt(0) === 65279 ? source2.slice(1) : source2;
...
function translateXmlParseError(error, source2, options) {
  if (!isXmlDomParseError(error) || options.role === void 0) {
    return error;
  }
  const message = normalizeXmlParseMessage(error.message);
  const location = createParseErrorLocation(source2, options.sourceName, error.locator);
  const suggestion = createParseSuggestion(options.role);
  if (options.role === "stylesheet") {
    return new XsltError(WEAVER_XML_STYLESHEET_PARSE_ERROR, `Stylesheet XML is not well-formed: ${message}.`, location, void 0, { suggestions: [suggestion] });
  }
  return new XsltError(WEAVER_XML_SOURCE_PARSE_ERROR, `Source XML is not well-formed: ${message}.`, location, void 0, { suggestions: [suggestion] });
}
...
function createParseErrorLocation(source2, sourceName, locator) {
  ...
  const offset = computeNormalizedOffset(source2, line, column);
  return {
...
function computeNormalizedOffset(source2, line, column) {
  const normalizedSource = source2.replace(/\r\n/g, "\n").replace(/[\r\u0085\u2028\u2029]/g, "\n");
...
// dist/runtime/xmlNodeHandles.js
...
function getXmlNodeHandleKind(node) {
  switch (node.nodeType) {
    ...
    case node.PROCESSING_INSTRUCTION_NODE:
      return "pi";
    default:
      throw new XsltError(XTSE0010, `Node type ${node.nodeType} cannot be represented as an XML trace handle.`);
  }
}
...
function getXmlNodePathSegment(node) {
  ...
  throw new XsltError(XTSE0010, `Node type ${node.nodeType} cannot be represented as an XML trace path segment.`);
}
...
function emitTraceEvent(trace, event) {
  ...
  const matched = activeBreakpoints.some((breakpoint) => breakpoint.on.includes(event.kind) && breakpoint.node.documentUri === event.node.documentUri && breakpoint.node.kind === event.node.kind && breakpoint.node.path === event.node.path);
  if (!matched) {
    return;
  }
...
function createTemplateFrame(template) {
...
function computeLevenshteinDistance(left, right) {
  ...
  for (let leftIndex = 1; leftIndex <= left.length; leftIndex += 1) {
    ...
    for (let rightIndex = 1; rightIndex <= right.length; rightIndex += 1) {
      ...
      previousRow[rightIndex] = Math.min((previousRow[rightIndex] ?? 0) + 1, (previousRow[rightIndex - 1] ?? 0) + 1, previousDiagonal + substitutionCost);
...
// dist/xpath/eval/comparisonHelpers.js
...
function matchesQualifiedNodeName(name, node, staticContext, isAttributeAxis) {
...
// dist/xpath/eval/contextHelpers.js
...
// dist/xpath/eval/flowExpressions.js
function createFlowExpressionEvaluator(dependencies) {
  ...
  function evaluateForExpression2(bindings, returnExpr, context) {
    return evaluateFlowBindings(bindings, context, (variables) => dependencies.evaluateExpression(returnExpr, {
      ...context,
      variables
    }));
  }
  function evaluateQuantifiedExpression2(quantifier, bindings, satisfiesExpr, context) {
    if (quantifier === "some") {
      return evaluateFlowBindings(bindings, context, (variables) => {
        const result = dependencies.effectiveBooleanValue(dependencies.evaluateExpression(satisfiesExpr, {
          ...context,
          variables
        }), satisfiesExpr.span);
...
// dist/xpath/eval/comparisonEvaluation.js
...
// dist/xpath/eval/pathEvaluation.js
function createPathEvaluator(dependencies) {
  ...
  function applyPathSegment(segment, input, context) {
    ...
    if (segment.kind === "functionCall") {
      dependencies.validateFunctionCallSignature(segment.callee.includes(":") ? segment.callee : `fn:${segment.callee}`, segment.arguments.length, segment.span);
    }
  ...
  function applyExpressionPathSegment(segment, input, context) {
    ...
    return input.flatMap((item, index) => dependencies.evaluateExpression(segment, {
      ...context,
      contextItem: item,
      contextPosition: index + 1,
      contextSize: size
...
// dist/xpath/eval/arityValidation.js
...
// dist/xpath/eval/scalarHelpers.js
...
// dist/xpath/eval/builtinFunctions.js
import { readFileSync } from "node:fs";
import { dirname, isAbsolute, resolve as resolvePath } from "node:path";
import { fileURLToPath } from "node:url";

// dist/xpath/eval/builtinFunctionSupport.js
...
function createBuiltinFunctionSupport(helpers) {
  ...
  function xpathSubstring(source2, roundedStart, roundedLength) {
    if (Number.isNaN(roundedStart) || roundedLength !== void 0 && Number.isNaN(roundedLength)) {
      return "";
    }
    const characters = Array.from(source2);
...
// dist/xpath/eval/builtinNodeFunctions.js
...
// dist/xpath/eval/builtinNumericFunctions.js
  ...
  function evaluateNumericBuiltinFunction(normalized, args, context, span) {
    switch (normalized) {
      ...
      case "fn:min": {
        ...
        return values.length === 0 ? [] : [support.createAtomicValueFromAtomized(values.reduce((current, candidate) => helpers.compareComparableValues(candidate, current) < 0 ? candidate : current))];
      }
      case "fn:max": {
        if (args.length !== 1 && args.length !== 2) {
          helpers.throwArityError(normalized, args.length, "1..2", span);
      ...
      case "fn:round": {
        ...
        return [createXdmNumber(support.roundToPrecision(helpers.requireSingleNumber(roundedItems, span), args.length === 2 ? helpers.requireSingleInteger(helpers.evaluateExpression(args[1], context), span, "Round precision") : 0))];
      }
      default:
        return void 0;
    }
...
// dist/xpath/eval/builtinSequenceFunctions.js
...
function toEcmaRegexFlags(flags, span, global = false, translatedPattern) {
  ...
  for (const flag of flags) {
    ...
    throw createRegexError(FORX0001, `Unsupported regular expression flag ${flag} in the current ECMAScript-compatible regex slice.`, span);
  }
...
function validateXPathRegexPattern(pattern, flags, span) {
  ...
  for (let index = 0; index < pattern.length; index += 1) {
    ...
    if (escaped) {
      ...
      if (char === "p" || char === "P") {
        throw createRegexError(FORX0002, `Invalid Unicode property escape \\${char} in XPath regular expression syntax.`, span);
      }
      if (inCharacterClass && /[0-9]/.test(char)) {
        throw createRegexError(FORX0002, `Invalid back-reference \\${char} inside a character class.`, span);
      }
      if (char === "0") {
        throw createRegexError(FORX0002, "Invalid back-reference \\0 in XPath regular expression syntax.", span);
      }
      ...
      if (/[A-Za-z]/.test(char) && !isSupportedXPathRegexEscape(char)) {
        throw createRegexError(FORX0002, `Unsupported XPath regular expression escape \\${char}.`, span);
      }
      escaped = false;
      if (!inCharacterClass) {
        canQuantify = true;
    ...
    if (char === "(") {
      if (pattern[index + 1] === "?") {
        if (pattern[index + 2] !== ":") {
          throw createRegexError(FORX0002, "Unsupported group construct in XPath regular expression syntax.", span);
        }
...
function resolveXPathNumericBackReference(pattern, startIndex, groupCount, openGroups, span) {
  ...
  for (let prefixLength = digits.length; prefixLength >= 1; prefixLength -= 1) {
    ...
    if (openGroups.includes(reference)) {
      throw createRegexError(FORX0002, `Invalid back-reference \\\\${reference} to a group that is not yet closed.`, span);
    }
    return {
      reference,
  ...
  throw createRegexError(FORX0002, `Invalid back-reference \\\\${digits} to a group that is not yet closed.`, span);
}
...
function tokenizeCharacterClassRawTokens(content) {
  ...
  for (let index = 0; index < content.length; index += 1) {
    ...
    if (char === "\\" && index + 1 < content.length) {
      ...
      rawTokens.push(escape === "d" || escape === "D" || escape === "w" || escape === "W" ? translateXPathRegexEscape(escape, true) : `\\${escape}`);
      index += 1;
      continue;
    }
    rawTokens.push(char);
...
// dist/xpath/eval/builtinStringFunctions.js
  ...
  function evaluateStringBuiltinFunction(normalized, args, context, span) {
    switch (normalized) {
      ...
      case "fn:substring": {
        ...
        const source2 = support.itemToStringValue(support.evaluateSingletonStringishArg(args[0], context, span, normalized), span);
        const start = support.xpathRound(helpers.requireSingleNumber(helpers.evaluateExpression(args[1], context), span));
        if (args.length === 2) {
          return [createXdmString(support.xpathSubstring(source2, start))];
        }
        const length = support.xpathRound(helpers.requireSingleNumber(helpers.evaluateExpression(args[2], context), span));
        return [createXdmString(support.xpathSubstring(source2, start, length))];
      }
      case "fn:substring-before": {
        helpers.requireArity(normalized, args, 2, span);
        const source2 = support.itemToStringValue(support.evaluateSingletonStringishArg(args[0], context, span, normalized), span);
        const search = support.itemToStringValue(support.evaluateSingletonStringishArg(args[1], context, span, normalized), span);
        const index = source2.indexOf(search);
        return [createXdmString(index < 0 ? "" : source2.slice(0, index))];
      }
      case "fn:substring-after": {
        helpers.requireArity(normalized, args, 2, span);
        const source2 = support.itemToStringValue(support.evaluateSingletonStringishArg(args[0], context, span, normalized), span);
        const search = support.itemToStringValue(support.evaluateSingletonStringishArg(args[1], context, span, normalized), span);
        const index = source2.indexOf(search);
        return [createXdmString(index < 0 ? "" : source2.slice(index + search.length))];
      }
      case "fn:codepoints-to-string": {
        helpers.requireArity(normalized, args, 1, span);
        return [
          createXdmString(support.codepointsToString(helpers.evaluateExpression(args[0], context), span))
        ];
      }
      ...
      case "fn:concat": {
        ...
        return [
          createXdmString(args.map((arg) => support.itemToStringValue(support.evaluateSingletonStringishArg(arg, context, span, normalized), span)).join(""))
        ];
      }
      case "fn:string-join": {
        if (args.length !== 1 && args.length !== 2) {
        ...
        return [
          createXdmString(items.map((item) => support.itemToStringValue(item, span)).join(separator))
        ];
      }
      case "fn:matches": {
        if (args.length !== 2 && args.length !== 3) {
      ...
      case "fn:replace": {
        ...
        return [
          createXdmString(input.replace(compileRegexRejectingZeroLengthMatches(pattern, flags, span), flags.includes("q") ? replacement.replace(/\$/g, "$$$$") : translateReplacementString(replacement, span)))
        ];
      }
      case "fn:tokenize": {
      ...
      case "fn:contains":
        helpers.requireArity(normalized, args, 2, span);
        return [
          createXdmBoolean(support.itemToStringValue(support.evaluateSingletonStringishArg(args[0], context, span, normalized), span).includes(support.itemToStringValue(support.evaluateSingletonStringishArg(args[1], context, span, normalized), span)))
        ];
      case "fn:starts-with":
        helpers.requireArity(normalized, args, 2, span);
        return [
          createXdmBoolean(support.itemToStringValue(support.evaluateSingletonStringishArg(args[0], context, span, normalized), span).startsWith(support.itemToStringValue(support.evaluateSingletonStringishArg(args[1], context, span, normalized), span)))
        ];
      case "fn:ends-with":
        helpers.requireArity(normalized, args, 2, span);
        return [
          createXdmBoolean(support.itemToStringValue(support.evaluateSingletonStringishArg(args[0], context, span, normalized), span).endsWith(support.itemToStringValue(support.evaluateSingletonStringishArg(args[1], context, span, normalized), span)))
        ];
      case "fn:upper-case": {
        helpers.requireArity(normalized, args, 1, span);
        return [
          createXdmString(support.itemToStringValue(support.evaluateSingletonStringishArg(args[0], context, span, normalized), span).toUpperCase())
        ];
      }
      case "fn:lower-case": {
        helpers.requireArity(normalized, args, 1, span);
        return [
          createXdmString(support.itemToStringValue(support.evaluateSingletonStringishArg(args[0], context, span, normalized), span).toLowerCase())
        ];
      }
      default:
        return void 0;
...
function createBuiltinFunctionEvaluator(helpers) {
  ...
  const { evaluateOptionalSingletonItemArg, evaluateSingletonStringishArg, itemToStringValue: itemToStringValue2 } = support;
  ...
  function evaluateFunctionCall2(callee, args, context, span) {
    ...
    switch (normalized) {
      case "fn:current":
        helpers.requireArity(normalized, args, 0, span);
        if (context.contextItem === null || context.contextItem === void 0) {
          return [];
        }
      ...
      case "fn:document": {
        ...
        if (uriItems.length !== 1 || uriItems[0]?.xdmKind !== "atomic") {
          throw helpers.createXPathError(XPTY0004, `Function ${normalized} requires a singleton URI argument.`, span, {
            functionName: normalized,
            expectedType: "singleton URI",
            actualType: helpers.describeItemsType(uriItems)
          });
        }
        const uri = String(uriItems[0].value);
      ...
      case "fn:deep-equal":
        helpers.requireArity(normalized, args, 2, span);
        return [
          createXdmBoolean(helpers.deepEqualSequences(helpers.evaluateExpression(args[0], context), helpers.evaluateExpression(args[1], context)))
        ];
      case "fn:QName":
        helpers.requireArity(normalized, args, 2, span);
        evaluateSingletonStringishArg(args[0], context, span, normalized);
        return [
          createXdmQName(itemToStringValue2(evaluateSingletonStringishArg(args[1], context, span, normalized)))
        ];
      case "map:entry": {
        helpers.requireArity(normalized, args, 2, span);
      ...
      case "fn:boolean":
        helpers.requireArity(normalized, args, 1, span);
        return [
          createXdmBoolean(helpers.effectiveBooleanValue(helpers.evaluateExpression(args[0], context), span))
        ];
      case "fn:not":
        helpers.requireArity(normalized, args, 1, span);
        return [
...
function resolveDocumentPath(uri, baseUri) {
...
var { requireContextItem, requireContextNode, isXdmNode, resolveVariableReference } = createContextHelpers({
  createXPathError,
  describeItemsType
});
function evaluateExpression(ast, context) {
  switch (ast.kind) {
    ...
    case "quantified":
      return [
        createXdmBoolean(evaluateQuantifiedExpression(ast.quantifier, ast.bindings, ast.satisfiesExpr, context))
      ];
    case "let":
    ...
    case "unary": {
      ...
      if (ast.operand.kind === "number" && isDecimalLiteralLexeme2(ast.operand.lexeme)) {
        return [
          createXdmNumber(ast.operator === "-" ? -operand : operand, normalizeSignedDecimalLiteralLexeme(ast.operator, ast.operand.lexeme))
        ];
      }
...
function evaluateBinaryExpression(operator, leftAst, rightAst, context, span) {
  ...
  if (operator === "||") {
    return [
      createXdmString(evaluateConcatOperandString(leftAst, context, span) + evaluateConcatOperandString(rightAst, context, span))
    ];
  }
  if (operator === "|") {
  ...
  if (operator === "except") {
    const right = new Set(requireNodeSequence(evaluateExpression(rightAst, context), rightAst.span).map((item) => item.node));
    return normalizeNodeSequence(requireNodeSequence(evaluateExpression(leftAst, context), leftAst.span).filter((item) => !right.has(item.node)));
  }
  if (operator === "eq" || operator === "ne" || operator === "lt" || operator === "le" || operator === "gt" || operator === "ge") {
    return compareValue(operator, evaluateExpression(leftAst, context), evaluateExpression(rightAst, context), span);
  }
  if (operator === "is" || operator === "<<" || operator === ">>") {
    return compareNodes(operator, evaluateExpression(leftAst, context), evaluateExpression(rightAst, context), span);
  }
  return [
    createXdmBoolean(compareGeneral(operator, evaluateExpression(leftAst, context), evaluateExpression(rightAst, context), span))
  ];
}
function evaluateNumericOperand(items, span) {
  const item = items[0];
  if (items.length !== 1 || item?.xdmKind !== "atomic" && item?.xdmKind !== "node" || item?.xdmKind === "atomic" && item.type !== "xs:double" && item.type !== "xs:integer") {
  ...
  const atomicItem = item;
  return atomicItem.value;
}
...
var { requireArity, validateFunctionCallSignature, throwArityError } = createArityValidationHelpers(createXPathError);
var { compareGeneral, atomizeItems, atomizedNumericValues, atomizedComparableValues, compareComparableValues, deepEqualSequences, atomizeSingleton, compareValueOperands } = createComparisonHelpers({
  createXPathError,
...
var { evaluateConcatOperandString, requireSingleNumber, requireSingleInteger, createNumberLiteralValue, normalizeSignedDecimalLiteralLexeme } = createScalarHelpers({
  evaluateExpression,
  createXPathError,
  describeItemsType,
  describeItemType
...
var { evaluateLetExpression, evaluateForExpression, evaluateQuantifiedExpression } = createFlowExpressionEvaluator({
  evaluateExpression,
  effectiveBooleanValue
});
...
// dist/xslt/eval/templateDispatch.js
...
function findBestMatchingTemplate(node, templates, staticContext, modeSet = []) {
  let bestTemplate;
...
function templateMatchesNode(template, node, staticContext, modeSet) {
  if (template.match === void 0) {
    return false;
  }
  ...
  if (simplePathMatch !== void 0) {
    return matchesSimpleTemplatePath(node, simplePathMatch.path, simplePathMatch.absolute);
  }
  const contextNode = node.nodeType === node.DOCUMENT_NODE ? node : node.parentNode ?? node.ownerDocument ?? node;
...
function tryGetSimpleTemplatePathMatch(match) {
  if (match.kind !== "path" || match.base !== void 0 || match.steps.length === 0) {
    return void 0;
  }
  const path = [];
...
function matchesSimpleTemplatePath(node, path, absolute) {
  let current = node;
...
function getDefaultTemplatePriorityForAst(ast) {
  if (ast.kind === "binary" && ast.operator === "|") {
    return Math.max(getDefaultTemplatePriorityForAst(ast.left), getDefaultTemplatePriorityForAst(ast.right));
  }
  if (ast.kind === "filter") {
    return 0.5;
  }
...
function createMatchContext(node, staticContext) {
  return {
    staticContext,
...
function asXdmNode(item) {
  return typeof item === "object" && item !== null && "node" in item ? item : void 0;
}
function templateMatchesMode(template, modeSet) {
  if (modeSet.length === 0) {
...
// dist/xslt/eval/transform.js
var import_xmldom2 = __toESM(require_lib(), 1);

// dist/xslt/eval/temporaryTree.js
...
// dist/xslt/eval/transform.js
var xmlSerializer = new import_xmldom2.XMLSerializer();
function runTransform(ir, sourceXml, options) {
  ...
  if (options.initialMode !== void 0) {
    throw new XsltError(XTDE0040, "Initial modes are not yet implemented in the current MVP+3 slice.", void 0, { mode: options.initialMode }, {
      suggestions: [
        {
          kind: "fix",
  ...
  const globalVariables = evaluateGlobalBindings(ir, ir.globalBindings, staticContext, createXdmNode(sourceDocument), options.parameters);
  ...
  const output = applyTemplatesToItems([createXdmNode(sourceDocument)], ir, staticContext, globalVariables, trace, sourceDocumentUri, void 0, []);
...
function applyTemplatesToItems(items, ir, staticContext, variables = /* @__PURE__ */ new Map(), trace, sourceDocumentUri = "<source-xml>", location, withParams = [], modes = [], attributeCollector) {
  if (items.some((item) => asXdmNode2(item) === void 0)) {
  ...
  return items.map((item, index) => {
    ...
    return applyTemplateToNode(nodeItem.node, ir, context, withParams, trace, sourceDocumentUri, modes, attributeCollector);
  }).join("");
}
function applyTemplateToNode(node, ir, context, withParams = [], trace, sourceDocumentUri = "<source-xml>", modes = [], attributeCollector) {
  ...
  if (template !== void 0) {
    ...
    try {
      return renderTemplate(template, withParams, ir, context, trace, sourceDocumentUri, attributeCollector);
    } catch (error) {
      throw prependXsltErrorFrame(error, createTemplateFrame2(template), createRelatedLocation("enclosing template", template.location));
    }
  }
  return renderBuiltInTemplate(node, ir, context.staticContext, context.variables, trace, sourceDocumentUri, modes);
}
function renderInitialTemplate(name, ir, context, trace, sourceDocumentUri = "<source-xml>") {
  ...
  if (template === void 0) {
    ...
    throw new XsltError(XTSE0010, `Initial template ${name} is not declared in the current stylesheet.`, void 0, {
  ...
  const contextNode = asXdmNode2(context.contextItem)?.node;
  ...
  try {
    ...
    throw prependXsltErrorFrame(error, createTemplateFrame2(template), createRelatedLocation("initial template", template.location));
  }
}
function renderInstructions(instructions, ir, context, trace, sourceDocumentUri = "<source-xml>", attributeCollector) {
  let output = "";
  ...
  for (const instruction of instructions) {
    ...
    output += renderInstruction(instruction, ir, currentContext, trace, sourceDocumentUri, attributeCollector);
  }
  return output;
}
function renderInstruction(instruction, ir, context, trace, sourceDocumentUri = "<source-xml>", attributeCollector) {
  switch (instruction.kind) {
    case "literalText":
      return instruction.disableOutputEscaping === true ? instruction.text : escapeText(instruction.text);
    case "comment":
      return `<!--${renderInstructions(instruction.body, ir, context, trace, sourceDocumentUri)}-->`;
    case "variable":
      return "";
    case "literalElement": {
      const collectedAttributes = /* @__PURE__ */ new Map();
      for (const attribute of instruction.attributes) {
        collectedAttributes.set(attribute.name, renderLiteralResultAttributeValue(attribute, context));
      }
      const body = renderInstructions(instruction.body, ir, context, trace, sourceDocumentUri, collectedAttributes);
      const attributes = [...collectedAttributes.entries()].map(([name, value]) => ` ${name}="${escapeAttribute(value)}"`).join("");
      return `<${instruction.name}${attributes}>${body}</${instruction.name}>`;
    }
    case "attribute": {
      if (attributeCollector === void 0) {
        throw new XsltError(XTSE0010, "xsl:attribute is only supported inside a literal result element in the current MVP+3 slice.", instruction.location, void 0, {
          suggestions: [
            {
              kind: "fix",
      ...
      const value = instruction.select === void 0 ? renderInstructions(instruction.body ?? [], ir, context, trace, sourceDocumentUri) : [...evaluate(instruction.select, context)].map(itemToStringValue).join(" ");
      attributeCollector.set(instruction.name, value);
      return "";
    }
    case "if": {
      try {
        return evaluateEffectiveBooleanValue(instruction.test, context) ? renderInstructions(instruction.body, ir, context, trace, sourceDocumentUri, attributeCollector) : "";
      } catch (error) {
        throw prependXsltErrorFrame(error, createInstructionFrame(`xsl:if test="${instruction.testText}"`, instruction.location), createRelatedLocation("containing instruction", instruction.location));
      }
    }
    case "choose": {
      for (const branch of instruction.whenBranches) {
        try {
          if (evaluateEffectiveBooleanValue(branch.test, context)) {
            return renderInstructions(branch.body, ir, context, trace, sourceDocumentUri, attributeCollector);
          }
        } catch (error) {
          throw prependXsltErrorFrame(error, createInstructionFrame(`xsl:when test="${branch.testText}"`, branch.location), createRelatedLocation("containing instruction", branch.location));
        }
      }
      return instruction.otherwiseBody === void 0 ? "" : renderInstructions(instruction.otherwiseBody, ir, context, trace, sourceDocumentUri, attributeCollector);
    }
    case "forEach": {
      try {
        ...
        return items.map((item, index) => renderInstructions(instruction.body, ir, {
          ...context,
          contextItem: item,
          contextPosition: index + 1,
          contextSize: items.length
        }, trace, sourceDocumentUri, attributeCollector)).join("");
      } catch (error) {
        throw prependXsltErrorFrame(error, createInstructionFrame(`xsl:for-each select="${instruction.selectText}"`, instruction.location), createRelatedLocation("containing instruction", instruction.location));
      }
    }
    case "callTemplate": {
      ...
      if (template === void 0) {
        ...
        throw new XsltError(XTSE0650, `Named template ${instruction.name} is not declared in the current stylesheet.`, instruction.location, {
          templateName: instruction.name
        }, suggestion === void 0 ? void 0 : { suggestions: [suggestion] });
      }
      try {
        return renderTemplate(template, instruction.withParams, ir, context, trace, sourceDocumentUri, attributeCollector);
      } catch (error) {
        throw prependXsltErrorFrame(error, createTemplateFrame2(template), createRelatedLocation("called template", template.location));
      }
    }
    case "valueOf": {
      try {
        ...
        throw prependXsltErrorFrame(error, createInstructionFrame(`xsl:value-of select="${instruction.selectText}"`, instruction.location), createRelatedLocation("containing instruction", instruction.location));
      }
    }
    case "copyOf": {
      try {
        const items = [...evaluate(instruction.select, context)];
        emitInstructionSelectEvents(items, trace, sourceDocumentUri, "xsl:copy-of", instruction.location);
        return items.map((item) => asXdmNode2(item) !== void 0 ? xmlSerializer.serializeToString(asXdmNode2(item).node) : escapeText(itemToStringValue(item))).join("");
      } catch (error) {
        throw prependXsltErrorFrame(error, createInstructionFrame(`xsl:copy-of select="${instruction.selectText}"`, instruction.location), createRelatedLocation("containing instruction", instruction.location));
      }
    }
    case "number": {
      try {
        ...
        throw prependXsltErrorFrame(error, createInstructionFrame(`xsl:number count="${instruction.countText}"`, instruction.location), createRelatedLocation("containing instruction", instruction.location));
      }
    }
    case "applyTemplates": {
      try {
        ...
        return applyTemplatesToItems(items, ir, context.staticContext, context.variables, trace, sourceDocumentUri, instruction.location, instruction.withParams, instruction.modes, attributeCollector);
      } catch (error) {
        throw prependXsltErrorFrame(error, createInstructionFrame(instruction.selectText === void 0 ? "xsl:apply-templates" : `xsl:apply-templates select="${instruction.selectText}"`, instruction.location), createRelatedLocation("caller instruction", instruction.location));
      }
    }
  }
}
function bindVariableInstruction(instruction, ir, context, trace, sourceDocumentUri = "<source-xml>") {
  try {
    ...
    throw prependXsltErrorFrame(error, createInstructionFrame(instruction.selectText === void 0 ? `xsl:variable name="${instruction.name}"` : `xsl:variable name="${instruction.name}" select="${instruction.selectText}"`, instruction.location), createRelatedLocation("containing instruction", instruction.location));
  }
}
function renderTemplate(template, withParams, ir, context, trace, sourceDocumentUri = "<source-xml>", attributeCollector) {
  ...
  return renderInstructions(template.body, ir, {
    ...context,
    variables
  }, trace, sourceDocumentUri, attributeCollector);
}
...
function throwMissingTemplateParam(param, providedNames) {
  ...
  throw new XsltError(XTDE0700, `Required template parameter $${param.name} was not supplied.`, param.location, {
    parameterName: param.name
  }, suggestion === void 0 ? void 0 : { suggestions: [suggestion] });
}
function getChildNodeItems(item) {
  const nodeItem = asXdmNode2(item);
  if (nodeItem === void 0) {
    return [];
  }
  const children = [];
...
function renderBuiltInTemplate(node, ir, staticContext, variables, trace, sourceDocumentUri = "<source-xml>", modes = []) {
  if (node.nodeType === node.DOCUMENT_NODE || node.nodeType === node.ELEMENT_NODE) {
    return applyTemplatesToItems(getChildNodeItems(createXdmNode(node)), ir, staticContext, variables, trace, sourceDocumentUri, void 0, [], modes);
  }
...
function renderNumberInstruction(instruction, context) {
  const node = asXdmNode2(context.contextItem)?.node;
  if (node === void 0) {
    return "";
  }
  const counts = getNumberingCounts(node, instruction.count, context.staticContext, instruction.level);
  if (counts.length === 0) {
    return "";
  }
  return formatNumberCounts(counts, instruction.format);
}
function getNumberingCounts(node, countPattern, staticContext, level) {
  if (level === "any") {
...
function countSinglePosition(node, countPattern, staticContext) {
  const parent = node.parentNode;
  if (parent === null) {
    return 1;
  }
...
function countAnyPreceding(node, countPattern, staticContext) {
...
function matchesNumberPattern(node, pattern, staticContext) {
  const contextNode = node.nodeType === node.DOCUMENT_NODE ? node : node.parentNode ?? node.ownerDocument ?? node;
  const context = {
    staticContext,
  ...
  try {
    return [...evaluate(pattern, context)].some((item) => asXdmNode2(item)?.node === node);
  } catch {
    return false;
  }
}
function formatNumberCounts(counts, format) {
  if (counts.length === 0) {
    return "";
  }
  if (counts.length === 1) {
  ...
  const parts = counts.map((count, index) => formatSingleCount(count, formatParts[index] ?? formatParts.at(-1) ?? "1"));
  return parts.join(".");
}
function formatSingleCount(count, format) {
  if (format === "i") {
  ...
  for (const binding of bindings) {
    ...
    const deferredBinding = {
      evaluate: () => {
        ...
        if (state === "evaluating") {
          throw new XsltError(XTDE0640, `Circular top-level ${binding.kind} dependency involving $${binding.name}.`, binding.location, { variableName: binding.name });
        }
        state = "evaluating";
        try {
          if (binding.kind === "param" && externalParameters.values.has(binding.name)) {
            ...
            throw new XsltError(XTDE0050, `Required stylesheet parameter $${binding.name} was not supplied.`, binding.location, {
          ...
          state = "pending";
          throw prependXsltErrorFrame(error, createInstructionFrame(binding.selectText === void 0 ? `xsl:${binding.kind} name="${binding.name}"` : `xsl:${binding.kind} name="${binding.name}" select="${binding.selectText}"`, binding.location), createRelatedLocation(`top-level ${binding.kind}`, binding.location));
        }
      }
    };
...
function createMissingStylesheetParameterSuggestion(expectedName, providedNames) {
  ...
  const nearest = providedNames.map((candidate) => ({
    candidate,
    distance: computeLevenshteinDistance(formatTemplateSuggestionName(expectedDisplayName), formatTemplateSuggestionName(candidate))
  })).sort((left, right) => left.distance - right.distance)[0];
  if (nearest === void 0 || nearest.distance > 2) {
    return void 0;
  }
...
function createApplyTemplatesNodeSequenceError(items, location) {
  return new XsltError(XPTY0004, "xsl:apply-templates requires a sequence of nodes.", location, {
    expectedType: "node()*",
...
function renderLiteralResultAttributeValue(attribute, context) {
  if (attribute.valueTemplate === void 0) {
    return attribute.value;
  }
  return attribute.valueTemplate.map((part) => renderAttributeValueTemplatePart(part, context)).join("");
}
function renderAttributeValueTemplatePart(part, context) {
  if (part.kind === "text") {
    return part.text;
  }
...
function transformCompiledStylesheet(ir, sourceXml, context = {}) {
  return runTransform(ir, sourceXml, context);
}

// dotnet/uri-resolver-test/Stylesheets/main.xsl.js
var stylesheet = { "version": "1.0", "xsltVersion": "3.0", "location": { "source": "main.xsl", "line": 1, "column": 1, "offset": 0, "endLine": 1, "endColumn": 2, "endOffset": 1 }, "namespaces": { "xsl": "http://www.w3.org/1999/XSL/Transform" }, "defaultElementNamespace": "", "globalBindings": [], "templates": [{ "location": { "source": "main.xsl", "line": 3, "column": 23, "offset": 105, "endLine": 3, "endColumn": 36, "endOffset": 118 }, "name": "from-imported", "modes": [], "params": [], "body": [{ "kind": "literalElement", "name": "imported", "attributes": [], "body": [{ "kind": "literalText", "text": "Imported says hi", "location": { "source": "main.xsl", "line": 4, "column": 15, "offset": 184, "endLine": 4, "endColumn": 16, "endOffset": 185 } }], "location": { "source": "main.xsl", "line": 4, "column": 5, "offset": 174, "endLine": 4, "endColumn": 6, "endOffset": 175 } }] }, { "location": { "source": "main.xsl", "line": 8, "column": 23, "offset": 256, "endLine": 8, "endColumn": 36, "endOffset": 269 }, "name": "from-included", "modes": [], "params": [], "body": [{ "kind": "literalElement", "name": "included", "attributes": [], "body": [{ "kind": "literalText", "text": "Included says hi", "location": { "source": "main.xsl", "line": 9, "column": 15, "offset": 335, "endLine": 9, "endColumn": 16, "endOffset": 336 } }], "location": { "source": "main.xsl", "line": 9, "column": 5, "offset": 325, "endLine": 9, "endColumn": 6, "endOffset": 326 } }] }, { "match": { "kind": "path", "absolute": true, "steps": [], "span": { "start": 0, "end": 1, "line": 1, "column": 1, "endLine": 1, "endColumn": 2 } }, "matchText": "/", "location": { "source": "main.xsl", "line": 13, "column": 24, "offset": 406, "endLine": 13, "endColumn": 25, "endOffset": 407 }, "modes": [], "params": [], "body": [{ "kind": "literalElement", "name": "root", "attributes": [], "body": [{ "kind": "copyOf", "select": { "kind": "path", "absolute": false, "base": { "kind": "functionCall", "callee": "document", "arguments": [{ "kind": "string", "lexeme": "'data/doc.xml'", "value": "data/doc.xml", "span": { "start": 9, "end": 23, "line": 1, "column": 10, "endLine": 1, "endColumn": 24 } }], "span": { "start": 0, "end": 24, "line": 1, "column": 1, "endLine": 1, "endColumn": 25 } }, "steps": [{ "kind": "step", "axis": "child", "nodeTest": { "kind": "nameTest", "name": "doc", "span": { "start": 25, "end": 28, "line": 1, "column": 26, "endLine": 1, "endColumn": 29 } }, "predicates": [], "span": { "start": 25, "end": 28, "line": 1, "column": 26, "endLine": 1, "endColumn": 29 } }], "span": { "start": 0, "end": 28, "line": 1, "column": 1, "endLine": 1, "endColumn": 29 } }, "selectText": "document('data/doc.xml')/doc", "location": { "source": "main.xsl", "line": 15, "column": 28, "offset": 497, "endLine": 15, "endColumn": 56, "endOffset": 525 } }, { "kind": "callTemplate", "name": "from-included", "withParams": [], "location": { "source": "main.xsl", "line": 16, "column": 32, "offset": 560, "endLine": 16, "endColumn": 45, "endOffset": 573 } }, { "kind": "callTemplate", "name": "from-imported", "withParams": [], "location": { "source": "main.xsl", "line": 17, "column": 32, "offset": 608, "endLine": 17, "endColumn": 45, "endOffset": 621 } }], "location": { "source": "main.xsl", "line": 14, "column": 5, "offset": 463, "endLine": 14, "endColumn": 6, "endOffset": 464 } }] }] };
var source = { path: "/Volumes/TDAS-R5/LocalSource/Weaver/dotnet/uri-resolver-test/Stylesheets/main.xsl", digest: "24061f8e" };
function transform(sourceXml, ctx = {}) {
  ctx = ctx.baseUri === void 0 ? { ...ctx, baseUri: source.path } : ctx;
  return transformCompiledStylesheet(stylesheet, sourceXml, ctx);
}
var main_xsl_default = { source, transform };
export {
  main_xsl_default as default,
  source,
  transform
};

//# sourceMappingURL=main.xsl.bundle.js.map
